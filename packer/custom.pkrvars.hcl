bios_type                = "seabios"
boot_command             = "<wait3>c<wait3>linux /casper/vmlinuz autoinstall ds='nocloud-net;s=http://{{ .HTTPIP }}:{{ .HTTPPort }}/' <enter><wait5>initrd /casper/initrd<wait5><enter>boot<wait5s><enter>"
boot_wait                = "10s"
bridge_name              = "vmbr0"
